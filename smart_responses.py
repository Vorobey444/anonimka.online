"""
Умная система генерации ответов для ботов
Множество вариантов, контекст, случайность - чтобы не спалиться!
"""

import random
from datetime import datetime

class SmartResponseGenerator:
    """Генератор естественных ответов с контекстом"""
    
    def __init__(self):
        self.conversation_history = {}
        self.last_topics = []
        
    # ============ ПРИВЕТСТВИЯ ============
    
    GREETINGS = {
        "casual": [
            "Йоу! Как дела?",
            "Здарова!",
            "Привет-привет!",
            "Хэй! Что нового?",
            "Салют!",
            "Ку!",
            "Дарова!",
        ],
        "friendly": [
            "Привет! 👋 Как настроение?",
            "Здравствуй! Рада видеть!",
            "Привет-привет! 🤗",
            "Хай! Как дела?",
            "Приветик!",
            "Здарова! Давно не виделись)",
        ],
        "emoji": [
            "Привет! 👋😊",
            "Хай! ✨",
            "Здравствуй! 🌸",
            "Йоу! 🔥",
            "Салют! 😎",
            "Приветик! 💫",
        ],
        "brief": [
            "Привет",
            "Здарова",
            "Хай",
            "Йоу",
            "Ку",
        ],
        "questions": [
            "Привет! Как дела?",
            "Здарова! Что делаешь?",
            "Хай! Давно тут?",
            "Привет! Что нового?",
        ]
    }
    
    # ============ ВОПРОСЫ "КАК ДЕЛА?" ============
    
    HOW_ARE_YOU = {
        "positive": [
            "Отлично! А у тебя?",
            "Всё супер! Ты как?",
            "Норм, общаюсь тут 😊",
            "Хорошо! А что у тебя?",
            "Пойдёт! Ты как?",
            "Да ничего, живём)",
            "Нормально, не жалуюсь",
        ],
        "neutral": [
            "Обычно. Ты как?",
            "Так себе, но норм",
            "Пойдёт, а ты?",
            "Работаю, учусь... Ты как?",
            "Да как обычно)",
        ],
        "with_question": [
            "Хорошо! А ты чем занят?",
            "Норм! Что делаешь?",
            "Отлично! Откуда ты?",
            "Пойдёт! Давно в чате?",
        ]
    }
    
    # ============ ПОДДЕРЖКА ============
    
    SUPPORT = {
        "empathy": [
            "Понимаю тебя...",
            "Держись! 💪",
            "Всё будет хорошо!",
            "Не переживай, всё наладится",
            "Мы рядом, не грусти",
            "Бывает такое у всех",
        ],
        "encourage": [
            "Не сдавайся!",
            "Ты справишься!",
            "Верю в тебя!",
            "Всё получится!",
            "Давай, не опускай руки!",
        ],
        "offer_help": [
            "Чем помочь?",
            "Расскажи подробнее",
            "Что случилось?",
            "Хочешь поговорить?",
            "Слушаю тебя",
        ]
    }
    
    # ============ СОГЛАСИЕ ============
    
    AGREEMENT = {
        "strong": [
            "Точно!",
            "Абсолютно согласен!",
            "Да, именно так!",
            "100%!",
            "Правильно говоришь!",
            "Полностью поддерживаю!",
        ],
        "casual": [
            "Ага",
            "Да",
            "Угу",
            "Точняк",
            "+1",
            "👍",
        ],
        "emoji": [
            "Да! 💯",
            "Точно! ✨",
            "Согласна! 😊",
            "Ага! 👍",
            "+100! 🔥",
        ]
    }
    
    # ============ НЕСОГЛАСИЕ ============
    
    DISAGREEMENT = {
        "soft": [
            "Хм, не уверен...",
            "Может быть...",
            "Не факт",
            "Сомневаюсь",
            "Не знаю, честно",
        ],
        "polite": [
            "У каждого своё мнение)",
            "Понимаю, но я думаю иначе",
            "Интересная точка зрения",
            "Может и так",
        ],
        "direct": [
            "Нет",
            "Не согласен",
            "Сомнительно",
            "Вряд ли",
        ]
    }
    
    # ============ ВОПРОСЫ ============
    
    QUESTIONS = {
        "about_person": [
            "Откуда ты?",
            "Давно тут?",
            "Как тебя зовут?",
            "Чем занимаешься?",
            "Сколько тебе лет?",
        ],
        "about_topic": [
            "А ты что думаешь?",
            "Какое твоё мнение?",
            "Ты как к этому относишься?",
            "Что скажешь?",
        ],
        "general": [
            "Что нового?",
            "Что делаешь?",
            "Планы какие?",
            "Чем интересуешься?",
        ]
    }
    
    # ============ ФРАЗЫ-ЗАПОЛНИТЕЛИ ============
    
    FILLERS = {
        "thinking": ["Хм...", "Эм...", "Ну...", "Типа..."],
        "agreement": ["Ага", "Да-да", "Угу", "Точно"],
        "surprise": ["Вау", "Ого", "Серьёзно?", "Правда?", "Не может быть"],
        "laugh": ["Ахах", "Хаха", "Ха", "Лол", "😂"],
    }
    
    # ============ КОНТЕКСТНЫЕ СВЯЗКИ ============
    
    CONNECTORS = {
        "continuation": ["Кстати,", "А ещё,", "К слову,", "Вообще,"],
        "change_topic": ["Кстати, другая тема:", "А знаешь что?", "Слушай,"],
        "return": ["Возвращаясь к теме:", "Кстати да,"],
    }
    
    # ============ ЭМОДЗИ ПО КОНТЕКСТУ ============
    
    EMOJI_BY_MOOD = {
        "happy": ["😊", "😄", "🙂", "😁", "🤗"],
        "excited": ["🔥", "✨", "💫", "🎉", "⚡"],
        "thinking": ["🤔", "🧐", "💭", "🤨"],
        "support": ["💪", "👍", "🙌", "❤️"],
        "casual": [")", ")", ")", "😎"],
    }
    
    # ============ ГЕНЕРАЦИЯ ОТВЕТОВ ============
    
    def generate_greeting(self, persona_style):
        """Генерирует приветствие в стиле персонажа"""
        greetings = self.GREETINGS.get(persona_style, self.GREETINGS["casual"])
        greeting = random.choice(greetings)
        
        # 30% шанс добавить follow-up вопрос
        if random.random() < 0.3:
            question = random.choice([
                "Как дела?",
                "Что делаешь?",
                "Давно тут?",
                "Что нового?",
            ])
            greeting = f"{greeting} {question}"
        
        return greeting
    
    def generate_how_are_you(self, persona_style):
        """Генерирует ответ на 'как дела?'"""
        category = random.choice(["positive", "neutral", "with_question"])
        responses = self.HOW_ARE_YOU[category]
        return random.choice(responses)
    
    def generate_support(self, persona_style):
        """Генерирует поддерживающий ответ"""
        category = random.choice(["empathy", "encourage", "offer_help"])
        responses = self.SUPPORT[category]
        response = random.choice(responses)
        
        # Добавляем эмодзи для эмоциональных стилей
        if persona_style in ["friendly", "emoji"]:
            emoji = random.choice(self.EMOJI_BY_MOOD["support"])
            response = f"{response} {emoji}"
        
        return response
    
    def generate_agreement(self, persona_style):
        """Генерирует согласие"""
        if persona_style == "brief":
            category = "casual"
        elif persona_style == "emoji":
            category = "emoji"
        else:
            category = random.choice(["strong", "casual"])
        
        return random.choice(self.AGREEMENT[category])
    
    def generate_question(self, persona_style):
        """Генерирует вопрос"""
        category = random.choice(["about_person", "about_topic", "general"])
        return random.choice(self.QUESTIONS[category])
    
    def add_natural_elements(self, response, persona_style):
        """Добавляет естественные элементы к ответу"""
        # 20% шанс добавить фразу-заполнитель в начало
        if random.random() < 0.2:
            filler_type = random.choice(["thinking", "agreement"])
            filler = random.choice(self.FILLERS[filler_type])
            response = f"{filler} {response.lower()}"
        
        # 15% шанс добавить эмодзи в конец
        if persona_style == "emoji" and random.random() < 0.5:
            mood = random.choice(["happy", "excited", "casual"])
            emoji = random.choice(self.EMOJI_BY_MOOD[mood])
            response = f"{response} {emoji}"
        elif persona_style in ["friendly", "casual"] and random.random() < 0.15:
            mood = random.choice(["happy", "casual"])
            emoji = random.choice(self.EMOJI_BY_MOOD[mood])
            response = f"{response} {emoji}"
        
        return response
    
    def generate_contextual_response(self, message_text, persona_style, context=None):
        """Генерирует контекстный ответ на основе сообщения"""
        message_lower = message_text.lower()
        
        # Анализ тональности
        positive_words = ["супер", "отлично", "круто", "класс", "кайф", "топ"]
        negative_words = ["плохо", "грустно", "скучно", "фигня", "хуёво"]
        question_words = ["как", "что", "где", "когда", "почему", "кто"]
        
        is_positive = any(word in message_lower for word in positive_words)
        is_negative = any(word in message_lower for word in negative_words)
        is_question = any(word in message_lower for word in question_words) or "?" in message_text
        
        # Генерируем базовый ответ
        if is_negative:
            response = self.generate_support(persona_style)
        elif is_positive:
            response = self.generate_agreement(persona_style)
        elif is_question:
            # На вопрос отвечаем или задаём встречный
            if random.random() < 0.6:
                response = self.generate_how_are_you(persona_style)
            else:
                response = self.generate_question(persona_style)
        else:
            # Общий ответ
            responses = [
                self.generate_greeting(persona_style),
                self.generate_agreement(persona_style),
                self.generate_question(persona_style),
            ]
            response = random.choice(responses)
        
        # Добавляем естественные элементы
        response = self.add_natural_elements(response, persona_style)
        
        return response
    
    def should_respond(self, message_text, user_id):
        """Определяет, нужно ли отвечать на сообщение"""
        message_lower = message_text.lower()
        
        # Всегда отвечаем на прямые обращения
        keywords = [
            "привет", "hello", "hi", "хай", "здравствуй", "салют",
            "как дела", "что делаешь", "кто тут", "есть кто",
            "помогите", "help", "скучно"
        ]
        
        if any(keyword in message_lower for keyword in keywords):
            return True
        
        # Проверяем историю с этим пользователем
        history_count = self.conversation_history.get(user_id, 0)
        
        # Если уже общались, больше шанс ответить
        if history_count > 0:
            return random.random() < 0.7  # 70% шанс
        
        # Если короткое сообщение, меньше шанс (возможно спам)
        if len(message_text) < 5:
            return random.random() < 0.2  # 20% шанс
        
        # Иначе средний шанс
        return random.random() < 0.4  # 40% шанс
    
    def update_conversation_history(self, user_id):
        """Обновляет историю разговора с пользователем"""
        self.conversation_history[user_id] = self.conversation_history.get(user_id, 0) + 1
    
    def get_random_starter(self, persona_style):
        """Генерирует случайное стартовое сообщение"""
        starters = [
            "Кто-нибудь тут?",
            "Всем привет!",
            "Как дела у всех?",
            "Что нового?",
            "Народ есть?",
            "Кто онлайн?",
            "Эй!",
            "Скучно...",
            "Че делаете?",
            "Поболтаем?",
        ]
        
        starter = random.choice(starters)
        return self.add_natural_elements(starter, persona_style)


# Глобальный генератор
response_generator = SmartResponseGenerator()
